<?php 
    ...
    if($version == 'print'){
        ...
        $years = $_POST['onlineRange'];
        
        $price = 30*($years);
        
    }
    
    $order_id = generateRandomOrder();
    
    ?>
<div class="main">
    <div class="container">
        <div class="row signup py-4">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-body">
                        <h3 class="card-title text-center pb-4">
                        <span style="color:#DB4729">Billing Details</span> / <span style="color:#1E695E">Shipping Details</span></h5>
                        <p style="color:#DB4729">
                            नाम:  <?php echo $details[0]->full_name ?>
                            <br>
                            इ-मेल:
                            <?php echo $details[0]->email ?><br> सम्पर्क नम्बर: <?php  echo $details[0]->phone_number?>
                        </p>
                        <div style="color:#1E695E" id="shippingdetails"></div>
                        <form action="/process.php" onSubmit="return false; validate();" method="post" id="address">
                            ...
                            <div class="subscription text-left mb-4">
                                <p class="text-left mb-0" style="color:#1E695E"><strong>देश</strong></p>
            ...
            <div class="col-md-4">
                <div class="card pricing">
                    <div class="card-body">
                        <h3 class="card-title text-center pb-4">
                        योजना विवरणहरु</h5>
                        <h6 class="card-price text-center mb-5"><span class="NPR"></span><span id="onlineAmount"> <?php  echo $price ?></span></h6>
                        <ul class="fa-ul">
                            <li><span class="fa-li"><i class="fas fa-check text-success"></i></span><strong>
                                संस्करण: <?php echo $_POST['version'] ?> 
                                </strong>
                            </li>
                            <li><span class="fa-li"><i class="fas fa-check text-success"></i></span><strong>
                                अवधि: <?php echo $years ?> years(s)
                                </strong>
                            </li>
                            <li><span class="fa-li"><i class="fas fa-check text-success"></i></span><strong>
                                विशेषताहरु: शिक्षक मासिक पत्रिकाको मुद्रित संस्करणको सदस्यता, तपाईंको ढोकाहरूमा डेलिभर गरिनेछ।
                                </strong>
                            </li>
                        </ul>
                        <form action="https://uat.esewa.com.np/epay/main" method="POST">
                            ...
                            <input value="http://oxygenaltitude.com/payment/esewa_payment_success?q=su&uid=<?php echo $_SESSION['fuser_id'] ?>&version=<?php echo $version ?>&years=<?php echo $years ?>" type="hidden" name="su">
                            <input value="http://merchant.com.np/page/esewa_payment_failed?q=fu" type="hidden" name="fu">
                            <input type="hidden" value="<?php echo $years ?>" name="years">
                            <button style="margin-left: 113px;" id="proceed" class="btn" type="submit" name="submit" id="submit" value="Proceed to pay"><img src='<?php echo FRONT_IMAGES_URL.'esewa-logo.png' ?>' style="width:100px;"></button>
                        </form>
                    </div>
                </div>
            </div>
...
<script>
    ...
    var data = $('#address :input').serializeArray();
        ...
        $.post($('#address').attr('action'), data, function(info){ $('#result').html(info); });
        
        $('#shippingdetails').load('/process/paymentSession.php', {
        });
